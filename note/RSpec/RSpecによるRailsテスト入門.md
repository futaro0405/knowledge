...
### システムスペックで使用するgem
前述のとおり、ここではブラウザの操作をシミュレートするために Capybara を使います。 Capybara を使うとリンクをクリックしたり、Web フォームを⼊⼒したり、画⾯の表⽰を検証 したりすることができます。Rails 5.1以降の Rails であれば Capybara はすでにインストールさ れています。なぜなら Capybara はシステムテストでも利⽤されるからです。念のため Gemfile を開き、テスト環境に Capybara が追加されていることを確認してください。